...
func TestInstance(t *testing.T) {
	...
	Convey("NewInstance creates a new instance with correct field data", t, func() {
		...
		t := NewInstance("workspaceID", "taskID", "instanceID", storage, InstanceConfig{doneCh, logger, tempFile, "driver", "command", []string{"args"}, env, "", []int{}, ""}).(*instance)
		So(t, ShouldNotBeNil)
	...
	Convey("Start starts a process", t, func() {
		...
		t := NewInstance("workspaceID", "taskID", "instanceID", storage, InstanceConfig{doneCh, logger, tempFile, "exec", "/bin/sh", []string{"-c", "echo 'foo'"}, env, "", []int{}, ""}).(*instance)
		So(t.isDone, ShouldEqual, false)
		err := t.Start()
		So(err, ShouldBeNil)
		<-doneCh
	...
	Convey("doneCh waits for new process to complete", t, func() {
		...
		start := time.Now()
		t := NewInstance("workspaceID", "taskID", "instanceID", storage, InstanceConfig{doneCh, logger, tempFile, "exec", "/bin/sh", []string{"-c", "sleep 1"}, env, "", []int{}, ""}).(*instance)
		So(t.isDone, ShouldEqual, false)
		err := t.Start()
		So(err, ShouldBeNil)
		So(t.isDone, ShouldEqual, false)
	...
	Convey("stderr has output", t, func() {
		...
		t := NewInstance("workspaceID", "taskID", "instanceID", storage, InstanceConfig{doneCh, logger, tempFile, "exec", "/bin/sh", []string{"-c", "sleep"}, env, "", []int{}, ""}).(*instance)
		So(t.isDone, ShouldEqual, false)
		err := t.Start()
		So(err, ShouldBeNil)
		So(t.isDone, ShouldEqual, false)
	...
	Convey("shell driver uses `/bin/sh -c`", t, func() {
		...
		start := time.Now()
		t := NewInstance("workspaceID", "taskID", "instanceID", storage, InstanceConfig{doneCh, logger, tempFile, "shell", "sleep", []string{"1"}, env, "", []int{}, ""}).(*instance)
		So(t.isDone, ShouldEqual, false)
		err := t.Start()
		So(err, ShouldBeNil)
		So(t.isDone, ShouldEqual, false)
	...
	Convey("Stop kills a running process", t, func() {
		...
		start := time.Now()
		t := NewInstance("workspaceID", "taskID", "instanceID", storage, InstanceConfig{doneCh, logger, tempFile, "exec", "/bin/sh", []string{"-c", "sleep 10"}, env, "", []int{}, ""}).(*instance)
		So(t.isDone, ShouldEqual, false)
		err := t.Start()
		So(err, ShouldBeNil)
		go func() {
